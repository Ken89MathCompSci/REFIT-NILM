"""
On-duration and time-of-day distribution analysis for APR-REFIT-House2-dataset/,
broken down per split (train / validation / test).

Copied unchanged from APR-REFIT-Scripts/analyze_refit_house2_distributions.py
into Sep-2026-House-2-scripts/ -- REFIT House 2's counterpart to
Sep-2026-UKDALE-scripts/analyze_apr_new_house5_distributions.py. Already
...
unlike House 5's microwave/washing_machine standby-floor artefacts.
...
Splits (all House 2, chronologically contiguous, no gap/overlap):
    train      2014-12-05 to 2015-01-03  (30 days)
    validation 2015-01-04 to 2015-01-10  (7 days)
    test       2015-01-11 to 2015-01-17  (7 days)

Per split, per appliance:
    1. Threshold at 10 W (this repo's established House-2 convention) to get
       a binary ON/OFF state per 8s sample.
...
Outputs (written into APR-REFIT-House2-dataset/):
...
def plot_split(results, split_name, out_dir=DATASET_DIR):
    ...
    fig.suptitle(f'APR-REFIT-House2-dataset -- ON-Duration & Time-of-Day '
...
def plot_comparison(all_results, out_dir=DATASET_DIR):
    ...
    fig, axes = plt.subplots(len(APPLIANCES), 2, figsize=(14, 4 * len(APPLIANCES)))
    fig.suptitle('APR-REFIT-House2-dataset -- Train vs Validation vs Test comparison', fontsize=13)
