"""
LSTM baseline for NILM -- APR-REFIT-House2-dataset/ splits.

Copied unchanged from APR-REFIT-Scripts/test_lstm_refit_house2_dataset.py into
Sep-2026-House-2-scripts/ -- REFIT House 2's counterpart to the
Sep-2026-UKDALE-scripts/ folder's test_lstm_apr_new_house5_dataset.py. Already
matches that folder's baseline pattern; only the threshold convention differs
(REFIT: uniform 10 W across all four clean-off channels vs House 5's
per-appliance 10/50/200/20 W).
...
same three changes described in test_gru_apr_new_house1_dataset.py:
  1. Reads APR-REFIT-House2-dataset/ CSVs instead of pickled UKDALE splits
     (House 2, train 2014-12-05..2015-01-03 / val 2015-01-04..01-10 / test 2015-01-11..01-17).
...
def train_lstm_on_appliance(data_dict, appliance_name, window_size=100,
                             ...
                             save_dir='models/lstm_refit_house2_dataset'):
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"lstm_refit_house2_{appliance_name}_best.pth")
            save_model(model,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"lstm_refit_house2_{appliance_name}_metrics.png"),
    ...
    with open(os.path.join(save_dir, f'lstm_refit_house2_{appliance_name}_history.json'),
...
def test_lstm_on_all_appliances(window_size=100, hidden_size=128, num_layers=2,
    ...
    base_save_dir = os.path.join(
        os.path.dirname(__file__), '..', 'models', f"lstm_refit_house2_dataset_{timestamp}")
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir, f"lstm_refit_house2_{appliance_name}_best.pth"),
    ...
    print(f"\nLSTM APR-REFIT-House2-dataset testing completed. Results saved to {base_save_dir}")
...
if __name__ == "__main__":
    print("Testing LSTM on APR-REFIT-House2-dataset...")
    ...
    print(f"\nSummary of LSTM testing on APR-REFIT-House2-dataset:")
