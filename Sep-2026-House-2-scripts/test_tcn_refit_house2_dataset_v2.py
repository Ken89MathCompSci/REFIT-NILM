"""
test_tcn_refit_house2_dataset_v2.py
=====================================
REFIT House 2 port of Sep-2026-UKDALE-scripts/test_tcn_apr_new_house5_dataset_v2.py
-- the "v2" gated-head TCN architecture, ported to this repo's
APR-REFIT-House2-dataset/ splits. The old un-gated TCN baseline for this
dataset (single MSE-only head, hand-picked 10 W threshold) lives at
APR-REFIT-Scripts/test_tcn_refit_house2_dataset.py; this v2 script replaces
it with the same two fixes documented for House 5:
...
ADAPTIVE PER-APPLIANCE THRESHOLDS (train-locked)
    ...
    values on the actual REFIT House 2 train split (2014-12-05 to 2015-01-03):
        dishwasher=43.0W, fridge=21.0W, microwave=43.0W, washing_machine=25.0W
    These differ from the uniform 10 W threshold used by
    test_gru/lnn/lstm_refit_house2_dataset.py -- same known threshold
...
POS_WEIGHT_CLAMP sized to REFIT House 2's actual class imbalance
    ...
    The upper clamp is set to 270 (compare House 5's raised 1600 for its far
    more extreme ~1587:1 microwave imbalance) -- grounded in a direct
    measurement of this dataset rather than reused from another house.
...
restore before test -- matches the House 5 v2 script unchanged.
"""
...
class GatedTCNModel(nn.Module):
    ...
    GatedTCNModel, adapted to REFIT House 2's pos_weight clamp.
    """
...
def train_tcn_on_appliance(data_dict, appliance_name, thresholds, window_size=100,
                            ...
                            save_dir='models/tcn_refit_house2_dataset_v2'):
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"tcn_refit_house2_v2_{appliance_name}_best.pth")
            save_model(model,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"tcn_refit_house2_v2_{appliance_name}_metrics.png"),
    ...
    with open(os.path.join(save_dir, f'tcn_refit_house2_v2_{appliance_name}_history.json'),
...
def test_tcn_on_all_appliances(window_size=100, num_channels=None, kernel_size=3,
    ...
    base_save_dir = os.path.join(
        os.path.dirname(__file__), '..', 'models', f"tcn_refit_house2_dataset_v2_{timestamp}")
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir, f"tcn_refit_house2_v2_{appliance_name}_best.pth"),
    ...
    print(f"\nGated TCN v2 APR-REFIT-House2-dataset testing completed. Results saved to {base_save_dir}")
...
if __name__ == "__main__":
    print("Testing Gated TCN v2 on APR-REFIT-House2-dataset...")
    ...
    print(f"\nSummary of Gated TCN v2 testing on APR-REFIT-House2-dataset:")
