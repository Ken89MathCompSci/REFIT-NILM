...
========================
REFIT House 2 port of Sep-2026-UKDALE-scripts/nilm_sfra_svm_house5.py -- an
SVM baseline for NILM on APR-REFIT-House2-dataset (House 2, 2014-12-05 to
2015-01-17). No equivalent SVM script existed for REFIT House 2 before this;
...
combined_pinn_lnn_refit_house2_dataset.py (raw, median, EMA, residual,
...
train-locked convention as combined_pinn_lnn_refit_house2_dataset.py).
Measured values on the actual train split: dishwasher=43.0W, fridge=21.0W,
microwave=43.0W, washing_machine=25.0W -- off:on ratios at these thresholds
are dishwasher ~12:1, fridge ~2:1, microwave ~261:1, washing_machine ~37:1,
all much less extreme than House 5's (up to ~1587:1 for microwave), so the
generic (1.0, 10.0) class-weight clamp from the House 5 script is left
unchanged here -- it only ever bites for microwave-scale imbalance, and this
dataset's worst ratio (~261:1) is still clamped down hard by design.
...
# ---------------------------------------------------------------------------
# Feature extraction  (identical to combined_pinn_lnn_refit_house2_dataset.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Adaptive thresholds  (identical to combined_pinn_lnn_refit_house2_dataset.py)
# ---------------------------------------------------------------------------
...
def load_data(dataset_dir: str) -> dict:
    print(f"Loading APR-REFIT-House2-dataset from '{dataset_dir}' ...")
