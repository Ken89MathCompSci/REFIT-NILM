"""
combined_pinn_lnn_refit_house2_dataset_v3.py
=============================================
...
combined_pinn_lnn_refit_house2_dataset.py (unchanged) and from
combined_pinn_lnn_refit_house2_dataset_v2.py (also left unchanged) rather
than editing either in place, so all three remain independently
runnable/comparable. Builds on v2's wider per-appliance heads (kept as-is
here) but replaces v2's fix to the classification-loss weighting -- see
point 6 below for why.

6. POS_WEIGHT-NORMALIZED EVENT LOSS + MODERATED LAMBDA_EVENT (new here,
   replaces v2's plain LAMBDA_EVENT=0.25 raise) -- motivated by what
   actually happened when the identical v2 fix was run on House 5 (this
   file's own v2 run hasn't been executed yet, only smoke-tested, so this
   is inference from House 5's result, not a direct House 2 before/after --
   worth confirming once this house's v2 is actually run to completion).
   On House 5, washing_machine (the appliance the fix targeted) improved
   dramatically (F1 0.015 -> 0.200, a ~13x gain), but macro F1 still
   dropped overall (0.389 -> 0.371) because the fix wasn't selective -- it
   made the gate fire more liberally for *every* appliance, not just the
   starved one. Microwave was the main casualty there: it was already
   reasonably precise before (0.409) even at low recall, and v2 traded
   almost all of that away (precision collapsed to 0.029, FP up 43x) for a
   modest recall gain. The likely mechanism, which applies here too:
   GatedEventLoss sums each appliance's pos_weight-scaled BCE term and
   divides by n_apps, but pos_weight only balances positive/negative
   *within* an appliance -- it doesn't equalize how much each appliance's
   loss *magnitude* contributes to the shared gradient. This house's own
   pos_weights span an even wider range (dishwasher ~12, fridge ~2,
   microwave ~261-270 clamped, washing_machine ~37) than House 5's
   (dishwasher ~13, fridge ~1, microwave ~56, washing_machine ~7), so if
   anything this imbalance-of-imbalances problem should be *more*
   pronounced here, not less. Two changes address this directly:
     - GatedEventLoss now divides each appliance's BCE term by its own
       pos_weight before summing, so every appliance contributes comparably
       to the gradient regardless of imbalance severity -- pos_weight still
       does its original job (within-appliance class balance) but no longer
       also sets between-appliance priority.
     - LAMBDA_EVENT moderated 0.25 -> 0.15 (down from v2, still above the
       original 0.05) since the normalization already changes the loss
       scale the outer multiplier is applied to; a smaller push should be
       enough now that appliances aren't competing on raw pos_weight
       magnitude.
   Both changes are bundled in one retrain rather than isolated -- if
   microwave/fridge/dishwasher still regress while washing_machine improves,
   that would suggest the normalization needs revisiting rather than just
   the lambda value.

Updated in Sep-2026-House-2-scripts/ with the ramp-epoch loss schedule from
Sep-2026-UKDALE-scripts/combined_pinn_lnn_apr_new_house5_dataset.py, on top
of the version already in APR-REFIT-Scripts/combined_pinn_lnn_refit_house2_dataset.py:

3. LINEAR RAMP AFTER WARMUP (new here) -- the physics/event loss terms used
   to switch on abruptly at WARMUP_EPOCHS (0 -> full lambda_phys/lambda_event
   in one step), which can destabilise a model that just spent 20 epochs
   fitting MSE alone. Now ramps lam linearly 0->1 over RAMP_EPOCHS=10 epochs
   after warmup, matching the House 5 script's fix. WARMUP_EPOCHS is also
   reduced 20 -> 10 here for the same reason documented there: a long warmup
   locks the model into "always OFF" before the gate ever sees a gradient.

4. POS_WEIGHT_CLAMP measured directly on this dataset's current train split
   (2014-12-05 to 2015-01-03) rather than reused from the House 1/House 5
   scripts. The old clamp here was (1.0, 50.0); measuring the actual
   microwave off:on ratio at its adaptive threshold on the *current*
   train split (see analyze_refit_house2_distributions.py) gives ~261:1 --
   the earlier window (before generate_refit_house2_dataset.py's window was
   improved for microwave coverage) was closer to 50:1, so 50.0 is now
   stale. Raised to (1.0, 270.0) so the gate's BCE loss for microwave isn't
   starved. Full measured ratios: dishwasher ~12:1, fridge ~2:1,
   microwave ~261:1, washing_machine ~37:1 -- all comfortably under the new
   ceiling, so only microwave is actually affected by the raise.

5. WIDER PER-APPLIANCE HEADS + HIGHER LAMBDA_EVENT (new here) -- motivated by
   a direct comparison against test_resnet_refit_house2_dataset_v2.py (a
   *separately trained* gated model per appliance, no shared trunk) run on
   this same dataset/window. ResNet v2's microwave precision was 0.71 vs
   this model's 0.015 -- a ~49x gap on the same data, same threshold, same
   general gate-then-power idea. That rules out "microwave is just
   unlearnable on this data" and points at the shared CNN+LiquidCell trunk
   feeding all four appliances into single nn.Linear(hidden*2, 1) probes:
   a bare linear layer has no capacity to reshape one shared 128-d
   representation into four appliance-specific decision boundaries for
   appliances with very different power/duration signatures. gate_heads and
   power_heads are now each Linear -> GELU -> Linear (HEAD_HIDDEN=32
   bottleneck) instead of a single Linear, giving every appliance room to
   specialize away from the shared representation. This alone is not
   sufficient, though: LAMBDA_EVENT (0.05) so heavily damps the gate's
   pos_weight-boosted BCE loss relative to MSE that a wider head has little
   gradient signal to actually use for a rare class like microwave (~0.4%
   ON). Raised to 0.25 (5x, same order of magnitude as the UKDALE House 5
   script's 0.05->0.30 raise for its own extreme imbalance) so the capacity
   increase has something to train on. Both changes are bundled in one
   retrain rather than isolated, since the hypothesis is that they're
   complementary (capacity without signal, or signal without capacity,
   likely both underperform on their own) -- if results still fall short,
   ablating them separately is the natural follow-up.
...
1. DATA SOURCE -- default --dataset-dir now points at APR-REFIT-House2-dataset/
   instead of medium_dataset/:
       APR-REFIT-House2-dataset/REFIT_HF_train.csv      (House 2, 2014-12-05 to 2015-01-03, 30 days)
       APR-REFIT-House2-dataset/REFIT_HF_validation.csv (House 2, 2015-01-04 to 2015-01-10, 7 days)
       APR-REFIT-House2-dataset/REFIT_HF_test.csv       (House 2, 2015-01-11 to 2015-01-17, 7 days)
...
2. PATH FIXES -- sys.path insert and default save_dir adjusted for this
   script living one directory deeper (Sep-2026-House-2-scripts/ instead of root).
...
LAMBDA_PHYS   = 0.01
LAMBDA_EVENT  = 0.15   # moderated from v2's 0.25 -- see header note 6.
                       # GatedEventLoss now normalizes each appliance's BCE
                       # term by its own pos_weight before summing, so this
                       # multiplier no longer needs to compensate for the
                       # loudest-pos_weight appliance dominating the shared
                       # gradient; a smaller push is enough.
EPSILON_W     = 50.0
...
HEAD_HIDDEN   = 32     # gate/power heads: Linear(hidden*2, HEAD_HIDDEN) -> GELU
                       ...
                       # probe -- see header note 5

THRESHOLD_DELTA   = 20.0
THRESHOLD_LOW_PCT = 0.05
THRESHOLD_MIN     = 10.0
CYCLING_P5_W      = 80.0
POS_WEIGHT_CLAMP  = (1.0, 270.0)  # raised from 50 -- microwave measures ~261:1
                                   # off:on at its adaptive threshold on the
                                   # current train split (see header note 4)
...
class CombinedPINNAdvancedLNN(nn.Module):
    ...
    Linear), not a single Linear probe -- see header note 5. This gives each
...
class GatedEventLoss(nn.Module):
    ...
    v3 change (see header note 6): each appliance's pos_weight-scaled BCE
    term is divided by that appliance's own pos_weight before summing.
    pos_weight still does its original job inside each appliance's own BCE
    (correcting for that appliance's positive/negative imbalance), but this
    stops the *magnitude* of pos_weight from also silently setting
    between-appliance priority in the shared gradient -- without the
    normalization, an appliance with pos_weight=270 contributes a much
    larger loss (and therefore much more gradient pull on the shared trunk)
    than one with pos_weight=12, for a comparable level of "wrongness",
    which is exactly the mechanism that let v2's higher LAMBDA_EVENT rescue
    a starved appliance on House 5 while overcorrecting an already-fine one.
    """
    ...
    def forward(self, gate_logits: torch.Tensor, y_scaled: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(self.n_apps):
            ...
            total = total + bce / self.pos_weights[i]
...
def train(data_dict: dict, save_dir: str,
    ...
    cfg = {
        'dataset': 'APR-REFIT-House2-dataset (H2, train 2014-12-05..2015-01-03 / '
                   'val 2015-01-04..01-10 / test 2015-01-11..01-17)',
    ...
    with open(os.path.join(save_dir, 'combined_pinn_lnn_refit_house2_dataset_v3_results.json'), 'w') as f:
...
def _plot_loss(history, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'combined_pinn_lnn_refit_house2_dataset_v3_loss.png'),
...
def _plot_metrics(history, test_metrics, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'combined_pinn_lnn_refit_house2_dataset_v3_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == '__main__':
    ...
    save_dir  = os.path.join(
        ...
        f'combined_pinn_lnn_refit_house2_dataset_v3_{timestamp}'
    )
