"""
GRU baseline for NILM -- APR-REFIT-House2-dataset/ splits.

Copied unchanged from APR-REFIT-Scripts/test_gru_refit_house2_dataset.py into
Sep-2026-House-2-scripts/ -- this is REFIT House 2's counterpart to the
Sep-2026-UKDALE-scripts/ folder. Nothing to port here: this script already
implements the baseline pattern used by that folder's test_gru/lnn/lstm
scripts (midpoint-aligned sequences, best-checkpoint restore before test).
The dataset-specific difference is REFIT's uniform 10 W threshold vs House
5's per-appliance thresholds (10/50/200/20 W) -- REFIT's four appliance
channels all read a clean 0 W when off, unlike House 5's microwave/washer
standby-floor artefacts.
...
1. DATA SOURCE -- reads directly from APR-REFIT-House2-dataset/ CSV files instead
   ...
   Appliances: dishwasher, fridge, microwave, washing_machine (all House 2 -- no
...
def train_gru_on_appliance(data_dict, appliance_name, window_size=100,
                            ...
                            save_dir='models/gru_refit_house2_dataset'):
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"gru_refit_house2_{appliance_name}_best.pth")
            save_model(model,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"gru_refit_house2_{appliance_name}_metrics.png"),
    ...
    with open(os.path.join(save_dir, f'gru_refit_house2_{appliance_name}_history.json'),
...
def test_gru_on_all_appliances(window_size=100, hidden_size=128, num_layers=2,
    ...
    base_save_dir = os.path.join(
        os.path.dirname(__file__), '..', 'models', f"gru_refit_house2_dataset_{timestamp}")
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir, f"gru_refit_house2_{appliance_name}_best.pth"),
    ...
    print(f"\nGRU APR-REFIT-House2-dataset testing completed. Results saved to {base_save_dir}\n")
...
if __name__ == "__main__":
    print("Testing GRU on APR-REFIT-House2-dataset...")
    ...
    print(f"\nSummary of GRU testing on APR-REFIT-House2-dataset:")
