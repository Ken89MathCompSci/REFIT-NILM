"""
test_resnet_refit_house2_dataset_v2.py
========================================
REFIT House 2 port of Sep-2026-UKDALE-scripts/test_resnet_apr_new_house5_dataset_v2.py
-- the "v2" gated-head ResNet architecture, ported to this repo's
APR-REFIT-House2-dataset/ splits. The old un-gated ResNet baseline for this
dataset (single MSE-only head, hand-picked 10 W threshold) lives at
APR-REFIT-Scripts/test_resnet_refit_house2_dataset.py; this v2 script
replaces it with the same two fixes documented for House 5:
...
ADAPTIVE PER-APPLIANCE THRESHOLDS (train-locked)
    ...
    combined_pinn_lnn_refit_house2_dataset.py and nilm_sfra_svm_house2.py,
    ...
    on the actual REFIT House 2 train split (2014-12-05 to 2015-01-03):
        dishwasher=43.0W, fridge=21.0W, microwave=43.0W, washing_machine=25.0W
    These differ from the uniform 10 W threshold used by
    test_gru/lnn/lstm_refit_house2_dataset.py -- REFIT's channels are clean
    ...
    gated scripts already documented for House 5.

POS_WEIGHT_CLAMP sized to REFIT House 2's actual class imbalance
    Measured directly from the current APR-REFIT-House2-dataset/REFIT_HF_train.csv
    at the adaptive thresholds above (off:on sample ratio):
        dishwasher ~12:1, fridge ~2:1, microwave ~261:1, washing_machine ~37:1
    Microwave is the extreme case here (compare House 5's ~1587:1 -- REFIT's
    imbalance is real but far less severe). The upper clamp is set to 270 so
    the gate's BCE loss isn't starved for microwave while the ratio it's
    actually clamping to stays grounded in a real measurement rather than
    reused from another house's numbers.
...
matches the House 5 v2 script unchanged.
"""
...
# Adaptive-threshold constants (identical formula to combined_pinn_lnn_refit_house2_dataset.py)
THRESHOLD_DELTA   = 20.0
THRESHOLD_LOW_PCT = 0.05
THRESHOLD_MIN     = 10.0
CYCLING_P5_W      = 80.0
POS_WEIGHT_CLAMP  = (1.0, 270.0)  # REFIT House 2 microwave measures ~261:1 off:on
                                   # at its adaptive threshold on the train split --
                                   # see header for the measured ratios of all four
                                   # appliances. Well below House 5's raised 1600.0.
...
def compute_adaptive_thresholds(df: pd.DataFrame) -> dict:
    """Identical formula to combined_pinn_lnn_refit_house2_dataset.py."""
    thresholds = {}
...
class GatedResNetModel(nn.Module):
    ...
    GatedResNetModel, adapted to REFIT House 2's pos_weight clamp.
    """
...
def train_resnet_on_appliance(data_dict, appliance_name, thresholds, window_size=100,
                               ...
                               save_dir='models/resnet_refit_house2_dataset_v2'):
    if layers is None:
        layers = [2, 2, 2]
    ...
    # as GatedEventLoss in combined_pinn_lnn_refit_house2_dataset.py
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"resnet_refit_house2_v2_{appliance_name}_best.pth")
            save_model(model,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"resnet_refit_house2_v2_{appliance_name}_metrics.png"),
    ...
    with open(os.path.join(save_dir, f'resnet_refit_house2_v2_{appliance_name}_history.json'),
...
def test_resnet_on_all_appliances(window_size=100, base_width=32, layers=None,
    ...
    base_save_dir = os.path.join(
        os.path.dirname(__file__), '..', 'models', f"resnet_refit_house2_dataset_v2_{timestamp}")
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir, f"resnet_refit_house2_v2_{appliance_name}_best.pth"),
    ...
    summary = {
        ...
        'version': 'v2',
        'dataset_splits': {
            'training':   {'house': 2, 'start': '2014-12-05', 'end': '2015-01-03'},
            'validation': {'house': 2, 'start': '2015-01-04', 'end': '2015-01-10'},
            'testing':    {'house': 2, 'start': '2015-01-11', 'end': '2015-01-17'}
        },
        'window_size': window_size,
    ...
    print(f"\nGated ResNet v2 APR-REFIT-House2-dataset testing completed. Results saved to {base_save_dir}")
...
if __name__ == "__main__":
    print("Testing Gated ResNet v2 on APR-REFIT-House2-dataset...")
    ...
    print(f"\nSummary of Gated ResNet v2 testing on APR-REFIT-House2-dataset:")
