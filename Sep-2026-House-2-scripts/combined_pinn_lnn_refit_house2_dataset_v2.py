"""
combined_pinn_lnn_refit_house2_dataset_v2.py
=============================================
...
combined_pinn_lnn_refit_house2_dataset.py (unchanged) rather than editing it
in place, so both remain independently runnable/comparable. This file adds
one thing on top of that script's ramp-epoch version: wider per-appliance
gate/power heads + a higher LAMBDA_EVENT (see point 5 below). Everything
else -- ramp-epoch schedule, POS_WEIGHT_CLAMP, adaptive thresholds, dataset
paths -- is identical to the unmodified script.
...
LAMBDA_PHYS   = 0.01
LAMBDA_EVENT  = 0.25   # raised from 0.05 -- see header note 5. A wider gate
                       # head needs enough gradient signal to actually use
                       # its added capacity; 0.05 left the pos_weight-boosted
                       # BCE term negligible next to MSE.
EPSILON_W     = 50.0
...
def train(data_dict: dict, save_dir: str,
    ...
    with open(os.path.join(save_dir, 'combined_pinn_lnn_refit_house2_dataset_v2_results.json'), 'w') as f:
...
def _plot_loss(history, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'combined_pinn_lnn_refit_house2_dataset_v2_loss.png'),
...
def _plot_metrics(history, test_metrics, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'combined_pinn_lnn_refit_house2_dataset_v2_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == '__main__':
    ...
    save_dir  = os.path.join(
        ...
        f'combined_pinn_lnn_refit_house2_dataset_v2_{timestamp}'
    )
