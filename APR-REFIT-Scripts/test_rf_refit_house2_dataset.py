"""
Random Forest baseline for NILM -- APR-REFIT-House2-dataset/ splits.
...
That transfer-learning machinery does NOT apply here: APR-REFIT-House2-dataset/
...
REFIT only records active power at 8s resolution (no V/I waveforms).
...
def train_rf_on_appliance(data_dict, appliance_name, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'rf_refit_house2_{appliance_name}_feature_importance.png'),
    ...
    with open(os.path.join(save_dir, f'rf_refit_house2_{appliance_name}_results.json'),
...
def main():
    ...
    base_save_dir = os.path.join(
        os.path.dirname(__file__), '..', 'models', f"rf_refit_house2_dataset_{timestamp}")
    ...
    print(f"\nRandom Forest APR-REFIT-House2-dataset testing completed. Results saved to {base_save_dir}\n")
