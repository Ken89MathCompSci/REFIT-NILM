"""
Physics-Informed LNN (PINN-LNN) for NILM — APR-REFIT-House2-dataset splits.

Derived from test_pinn_lnn_apr_dataset.py with one change:

1. DATA SOURCE — reads directly from APR-REFIT-House2-dataset/ CSV files:
...
# Uniform 10 W threshold — valid for all appliances with House 2 test data.
THRESHOLDS = {
    'dishwasher':      10.0,
    'fridge':          10.0,
    'microwave':       10.0,
...
def train_pinn_model(data_dict, save_dir,
    ...
    config = {
        ...
        'model': 'PhysicsInformedLiquidNetworkModel',
        'description': 'adaptive LNN (adaptive tau + input gate) + L_phys; all House 2, 10 W threshold',
    ...
    with open(os.path.join(save_dir, 'pinn_lnn_refit_house2_dataset_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_refit_house2_dataset_loss.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('PINN-LNN APR-REFIT-House2-dataset -- Per-Appliance Val Metrics', fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_refit_house2_dataset_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    save_dir  = os.path.join(
        ...
        f"pinn_lnn_refit_house2_dataset_{timestamp}"
    )
