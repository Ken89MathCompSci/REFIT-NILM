"""
nilm_sfra_svm_house3.py
========================
REFIT House 3 port of Sep-2026-UKDALE-scripts/nilm_sfra_svm_house5.py -- an
SVM baseline for NILM on APR-REFIT-House3-dataset (House 3, 2015-02-10 to
2015-03-25). No equivalent SVM script existed for REFIT House 3 before this;
...
combined_pinn_lnn_refit_house3_dataset.py (raw, median, EMA, residual,
...
train-locked convention as combined_pinn_lnn_refit_house3_dataset.py).
Measured values on the actual train split: dishwasher=21.0W, fridge=97.0W,
microwave=28.0W, washing_machine=21.0W -- off:on ratios at these thresholds
are dishwasher ~20:1, fridge ~8:1, microwave ~187:1, washing_machine ~18:1,
...
here -- it bites hardest for microwave (187 -> 10), same as it does across
...
# ---------------------------------------------------------------------------
# Feature extraction  (identical to combined_pinn_lnn_refit_house3_dataset.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Adaptive thresholds  (identical to combined_pinn_lnn_refit_house3_dataset.py)
# ---------------------------------------------------------------------------
...
def load_data(dataset_dir: str) -> dict:
    print(f"Loading APR-REFIT-House3-dataset from '{dataset_dir}' ...")
...
def main(dataset_dir: str = DEFAULT_DATASET_DIR, tune: bool = False,
    ...
    out_path  = os.path.join(dataset_dir, f'svm_house3_results_{timestamp}.json')
