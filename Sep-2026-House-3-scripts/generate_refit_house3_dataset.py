"""
Generates APR-REFIT-House3-dataset/ -- multi-day REFIT House 3 splits, same
...
REFIT House 3 appliance channels (processed REFITPowerData111215/House3.csv),
confirmed against the official MetaData_Tables.xlsx (not inferred from power
signatures):
    Columns (no header): Unix, Aggregate, App1-App9
    fridge           -> col 2  (Appliance2 = Fridge-Freezer, Whirlpool)
    washing_machine  -> col 6  (Appliance6 = Washing Machine)
    dishwasher       -> col 5  (Appliance5 = Dishwasher, Bosch)
    microwave        -> col 8  (Appliance8 = Microwave, Panasonic)
    (col1 = Toaster, col3 = Freezer, col4 = Tumble Dryer, col7 = TV,
    col9 = Kettle -- all skipped)
...
Full history is 2013-09-25 to 2015-06-02 (~614 days). The selected window
landed on 2015-02-10 to 2015-03-25, with a worst-case per-split activation
floor of 1,567 samples (moderately richer than House 2's 1,151; washing
machine here is heavily fragmented -- 37,650 threshold-crossing segments
across the full history, median duration 0.4 min -- consistent with a
multi-stage wash cycle repeatedly dipping below the 10W threshold, same
pattern already seen in House 2's washing_machine channel).
...
OUT_DIR = os.path.join(BASE, "..", "APR-REFIT-House3-dataset")
os.makedirs(OUT_DIR, exist_ok=True)

# House 3 channel mapping, per MetaData_Tables.xlsx "House 3" sheet:
#   Appliance2 (col2) = Fridge-Freezer    -> fridge
#   Appliance5 (col5) = Dishwasher        -> dishwasher
#   Appliance6 (col6) = Washing Machine   -> washing_machine
#   Appliance8 (col8) = Microwave         -> microwave
H3_COL_MAP = {
    "aggregate":       "agg",
    "fridge":          "app2",
    "washing_machine": "app6",
    "dishwasher":      "app5",
    "microwave":       "app8",
}
...
def scan_best_window(raw_df: pd.DataFrame) -> dict:
    ...
    if n_days_total < total_days:
        raise ValueError(
            f"REFIT House 3 has only {n_days_total} days of data; "
...
def generate_split(raw_df: pd.DataFrame, name: str,
    ...
    print(f"\n{'='*60}")
    print(f"Processing split: {name}  (House 3, {start} -> {end})")
    print(f"{'='*60}")

    cols = {}
    for label, col in H3_COL_MAP.items():
...
if __name__ == "__main__":
    csv_path = os.path.join(REFIT, "House3.csv")
    ...
    print(f"Loading REFIT House 3: {csv_path}")
