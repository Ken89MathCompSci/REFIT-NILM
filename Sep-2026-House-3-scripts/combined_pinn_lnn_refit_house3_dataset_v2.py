"""
combined_pinn_lnn_refit_house3_dataset_v2.py
=============================================
Dual-head PINN + Gated-Detection LNN for NILM -- APR-REFIT-House3-dataset/ splits.

Kept as a separate file from combined_pinn_lnn_refit_house3_dataset.py
(unchanged) rather than editing it in place, so both remain independently
runnable/comparable -- same pattern as the House 2/House 5 _v2/_v3 files.

WHY THIS FILE EXISTS -- House 3 is a different failure mode than House 2/5
   Running the full 6-model comparison on House 3 (unmodified scripts)
   showed every model clustered in a mediocre F1 band (0.17-0.36), with
   AB-PILNN the *worst* of the six (0.167) -- not just weak on one
   appliance like House 2 (microwave) or House 5 (washing_machine), but
   uniformly behind GRU/LSTM/SVM/ResNet v2/TCN v2. When every architecture
   underperforms together, that points at the dataset/evaluation pipeline
   itself, not any one model's architecture. Pulling AB-PILNN's per-appliance
   breakdown confirmed two *separate* problems, fixed independently below:

FIX 1 -- FRIDGE THRESHOLD BUG (CYCLING_P5_W, data/evaluation fix, not a
model fix -- affects every model's fridge score on this house, not just
AB-PILNN's)
   AB-PILNN's fridge F1 was 0.034 (recall 0.022, missing 97.8% of true ON
   time) -- by far the worst fridge result seen on any house/model so far
   (House 2/5 fridge F1 ranged 0.55-0.87; fridge is normally the *easiest*
   appliance). Inspecting the raw channel explains why: House 3's fridge
   compressor's actual running power clusters tightly around 87W (105,461
   samples in an 8s-resampled 30-day train split), but
   compute_adaptive_thresholds() computed a 97W cutoff for this appliance
   -- landing *above* the bulk of genuine ON-time, not below it. Root
   cause: the formula's p5(nonzero)+20W branch assumes a gradual low-power
   tail to clear; House 3's fridge instead has a clean bimodal split (50.5%
   exactly 0W, then a narrow ~67-116W cluster), so p5=77W already sits near
   the cluster and +20W overshoots through its peak. The formula's other
   branch (flat THRESHOLD_MIN=10W, used when p5 > CYCLING_P5_W=80W) exists
   precisely for this "clean, tightly-clustered cycling appliance" case --
   House 3's fridge narrowly misses it (p5=77W vs the 80W cutoff).
   Fix: CYCLING_P5_W lowered 80.0 -> 75.0 in this file only, which flips
   fridge into the flat-10W branch (77 > 75) without touching any other
   appliance on this house (dishwasher/microwave/washing_machine all have
   p5 well under 75) or any other house's already-verified thresholds
   (their p5 values sit either well below 75 or already above 80).

FIX 2 -- DISHWASHER OVER-TRIGGERING (shared-trunk fix, adopts the *v3*
lesson from House 2/5, not the disproven v2 approach)
   AB-PILNN's dishwasher precision was 0.076 against recall 0.682 (20,403
   false positives vs 1,666 true positives) -- a genuine over-triggering
   problem, not threshold-related (dishwasher's own 21W threshold is
   unremarkable). This is the same class of shared-trunk imbalance-of-
   imbalances problem diagnosed on House 2 (microwave starved) and House 5
   (washing_machine starved, then overcorrected by a blunt LAMBDA_EVENT
   raise in that house's v2). Rather than repeat the v2 mistake here, this
   file goes straight to the v3-style fix validated conceptually on House
   2/5: gate_heads/power_heads widened to Linear -> GELU -> Linear
   (HEAD_HIDDEN=32), GatedEventLoss normalizes each appliance's BCE term by
   its own pos_weight before summing (so no single appliance's imbalance
   severity dominates the shared gradient), and LAMBDA_EVENT set to 0.15
   (not 0.05, not v2's 0.25).

POS_WEIGHT_CLAMP measured directly on House 3's own train split
   (2015-02-10 to 2015-03-11), not reused from House 2. Measured off:on
   ratios at the adaptive threshold on this train split: dishwasher ~20:1,
   fridge ~8:1, microwave ~187:1, washing_machine ~18:1. In between House
   2's ~261:1 and House 5's ~56:1 worst-case microwave imbalance.
   Clamped to (1.0, 200.0) accordingly. Unchanged from the original script.

DATA SOURCE -- default --dataset-dir points at APR-REFIT-House3-dataset/:
    APR-REFIT-House3-dataset/REFIT_HF_train.csv      (House 3, 2015-02-10 to 2015-03-11, 30 days)
    APR-REFIT-House3-dataset/REFIT_HF_validation.csv (House 3, 2015-03-12 to 2015-03-18, 7 days)
    APR-REFIT-House3-dataset/REFIT_HF_test.csv       (House 3, 2015-03-19 to 2015-03-25, 7 days)
...
fridge compressor cycle and was carried over unchanged for House 3; run
analyze_refit_house3_distributions.py if you want this house's own
per-appliance ON-duration distribution to double-check WIN is still generous
enough (House 3's fridge alone runs at 54% duty over its full history, per
the earlier full-house scan).
"""
...
LAMBDA_PHYS   = 0.01
LAMBDA_EVENT  = 0.15   # v3-style value (not v2's disproven 0.25, not the
                       # original 0.05) -- see header FIX 2. Paired with
                       # GatedEventLoss's pos_weight normalization below.
EPSILON_W     = 50.0
...
HEAD_HIDDEN   = 32     # gate/power heads: Linear(hidden*2, HEAD_HIDDEN) -> GELU
                       ...
                       # probe -- see header FIX 2

THRESHOLD_DELTA   = 20.0
THRESHOLD_LOW_PCT = 0.05
THRESHOLD_MIN     = 10.0
CYCLING_P5_W      = 75.0  # lowered from 80.0 -- see header FIX 1. House 3's
                           # fridge has p5(nonzero)=77W, narrowly missing the
                           # original 80W "clean cycling" cutoff and getting
                           # a 97W threshold (p5+20) that sits *above* the
                           # compressor's actual running power (~87W peak
                           # cluster). Lowering to 75 flips fridge into the
                           # flat THRESHOLD_MIN=10W branch instead. Verified
                           # this doesn't affect dishwasher/microwave/
                           # washing_machine on this house (all p5 << 75).
POS_WEIGHT_CLAMP  = (1.0, 200.0)  # House 3 microwave measures ~187:1 off:on at
                                   # its adaptive threshold on the train split
                                   # (see header) -- between House 2's 270.0
                                   # and House 5's 60.0. Unchanged from v1.

APPLIANCES  = ['dishwasher', 'fridge', 'microwave', 'washing_machine']
AGG_COL     = 'aggregate'

DEFAULT_DATASET_DIR = os.path.join(os.path.dirname(__file__), '..', 'APR-REFIT-House3-dataset')
...
def load_data(dataset_dir: str) -> dict:
    print(f"Loading APR-REFIT-House3-dataset CSV data from '{dataset_dir}' ...")
...
class CombinedPINNAdvancedLNN(nn.Module):
    ...
    Linear), not a single Linear probe -- see header FIX 2. This gives each
...
class GatedEventLoss(nn.Module):
    ...
    v2 change (see header FIX 2): each appliance's pos_weight-scaled BCE
    ...
    normalization, microwave (pos_weight~187-200 here) contributes a much
    larger loss, and therefore much more gradient pull on the shared trunk,
    than dishwasher (pos_weight~20), for a comparable level of "wrongness".
    """
...
def train(data_dict: dict, save_dir: str,
    ...
    cfg = {
        'dataset': 'APR-REFIT-House3-dataset (H3, train 2015-02-10..2015-03-11 / '
                   'val 2015-03-12..2015-03-18 / test 2015-03-19..2015-03-25)',
    ...
    with open(os.path.join(save_dir, 'combined_pinn_lnn_refit_house3_dataset_v2_results.json'), 'w') as f:
...
def _plot_loss(history, save_dir):
    ...
    fig.suptitle('CombinedPINNAdvancedLNN (APR-REFIT-House3-dataset) -- Loss Curves')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'combined_pinn_lnn_refit_house3_dataset_v2_loss.png'),
...
def _plot_metrics(history, test_metrics, save_dir):
    ...
    fig.suptitle('CombinedPINNAdvancedLNN (APR-REFIT-House3-dataset) -- Per-Appliance Validation Metrics')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'combined_pinn_lnn_refit_house3_dataset_v2_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == '__main__':
    ...
    save_dir  = os.path.join(
        ...
        f'combined_pinn_lnn_refit_house3_dataset_v2_{timestamp}'
    )
