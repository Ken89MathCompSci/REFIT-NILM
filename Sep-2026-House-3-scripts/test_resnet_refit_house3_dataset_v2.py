"""
test_resnet_refit_house3_dataset_v2.py
========================================
REFIT House 3 port of Sep-2026-UKDALE-scripts/test_resnet_apr_new_house5_dataset_v2.py
-- the "v2" gated-head ResNet architecture, ported to this repo's
APR-REFIT-House3-dataset/ splits. Unlike House 2 (which has both an old
un-gated v1 script and this v2 gated version), no v1 ResNet baseline exists
for House 3 -- this v2 script is ported directly from House 2's v2 script,
...
ADAPTIVE PER-APPLIANCE THRESHOLDS (train-locked)
    ...
    combined_pinn_lnn_refit_house3_dataset.py and nilm_sfra_svm_house3.py,
    ...
    on the actual REFIT House 3 train split (2015-02-10 to 2015-03-11):
        dishwasher=21.0W, fridge=97.0W, microwave=28.0W, washing_machine=21.0W
    These differ from the uniform 10 W threshold used by
    test_gru/lnn/lstm_refit_house3_dataset.py -- REFIT's channels are clean
    ...
    Fridge's threshold here (97.0W) is notably higher than House 2's/House
    5's (~21-22W) -- House 3's fridge-freezer draws more at its low-power
    tail, so the p5+20W formula lands higher; this is expected, data-driven
    behaviour, not a bug.
    This is the same known threshold inconsistency between baseline and
    gated scripts already documented for House 2.

POS_WEIGHT_CLAMP sized to REFIT House 3's actual class imbalance
    Measured directly from the current APR-REFIT-House3-dataset/REFIT_HF_train.csv
    at the adaptive thresholds above (off:on sample ratio):
        dishwasher ~20:1, fridge ~8:1, microwave ~187:1, washing_machine ~18:1
    Microwave is the extreme case here, in between House 2's ~261:1 and
    House 5's ~56:1. The upper clamp is set to 200 so the gate's BCE loss
    isn't starved for microwave while the ratio it's actually clamping to
    stays grounded in a real measurement of this dataset rather than reused
    from another house's numbers.
...
DATASET_DIR = os.path.join(os.path.dirname(__file__), '..', 'APR-REFIT-House3-dataset')
APPLIANCES  = ['dishwasher', 'fridge', 'microwave', 'washing_machine']

# Adaptive-threshold constants (identical formula to combined_pinn_lnn_refit_house3_dataset.py)
THRESHOLD_DELTA   = 20.0
THRESHOLD_LOW_PCT = 0.05
THRESHOLD_MIN     = 10.0
CYCLING_P5_W      = 80.0
POS_WEIGHT_CLAMP  = (1.0, 200.0)  # REFIT House 3 microwave measures ~187:1 off:on
...
def compute_adaptive_thresholds(df: pd.DataFrame) -> dict:
    """Identical formula to combined_pinn_lnn_refit_house3_dataset.py."""
    thresholds = {}
...
class GatedResNetModel(nn.Module):
    ...
    GatedResNetModel, adapted to REFIT House 3's pos_weight clamp.
    """
...
def train_resnet_on_appliance(data_dict, appliance_name, thresholds, window_size=100,
                               ...
                               save_dir='models/resnet_refit_house3_dataset_v2'):
    if layers is None:
        layers = [2, 2, 2]
    ...
    # as GatedEventLoss in combined_pinn_lnn_refit_house3_dataset.py
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"resnet_refit_house3_v2_{appliance_name}_best.pth")
            save_model(model,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"resnet_refit_house3_v2_{appliance_name}_metrics.png"),
                dpi=150, bbox_inches='tight')
    plt.close()

    config = {
        'appliance': appliance_name,
        'dataset': 'APR-REFIT-House3-dataset',
        'model': 'GatedResNetModel',
        'version': 'v2',
        'fixes': {
    ...
    with open(os.path.join(save_dir, f'resnet_refit_house3_v2_{appliance_name}_history.json'),
...
def test_resnet_on_all_appliances(window_size=100, base_width=32, layers=None,
    ...
    base_save_dir = os.path.join(
        os.path.dirname(__file__), '..', 'models', f"resnet_refit_house3_dataset_v2_{timestamp}")
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir, f"resnet_refit_house3_v2_{appliance_name}_best.pth"),
    ...
    summary = {
        ...
        'version': 'v2',
        'dataset_splits': {
            'training':   {'house': 3, 'start': '2015-02-10', 'end': '2015-03-11'},
            'validation': {'house': 3, 'start': '2015-03-12', 'end': '2015-03-18'},
            'testing':    {'house': 3, 'start': '2015-03-19', 'end': '2015-03-25'}
        },
        'window_size': window_size,
    ...
    print(f"\nGated ResNet v2 APR-REFIT-House3-dataset testing completed. Results saved to {base_save_dir}")
...
if __name__ == "__main__":
    print("Testing Gated ResNet v2 on APR-REFIT-House3-dataset...")
    ...
    print(f"\nSummary of Gated ResNet v2 testing on APR-REFIT-House3-dataset:")
