"""
On-duration and time-of-day distribution analysis for APR-REFIT-House3-dataset/,
broken down per split (train / validation / test).

Ported from APR-REFIT-Scripts/analyze_refit_house2_distributions.py to
House 3's own dataset -- REFIT House 3's counterpart to
...
Splits (all House 3, chronologically contiguous, no gap/overlap):
    train      2015-02-10 to 2015-03-11  (30 days)
    validation 2015-03-12 to 2015-03-18  (7 days)
    test       2015-03-19 to 2015-03-25  (7 days)

Per split, per appliance:
    1. Threshold at 10 W (this repo's established uniform-threshold
       convention, same as House 2/5/10) to get a binary ON/OFF state per
       8s sample.
...
for House 3 rather than assuming it's fine just because the overall
min-activation score is decent.

Outputs (written into APR-REFIT-House3-dataset/):
...
def plot_split(results, split_name, out_dir=DATASET_DIR):
    ...
    fig.suptitle(f'APR-REFIT-House3-dataset -- ON-Duration & Time-of-Day '
...
def plot_comparison(all_results, out_dir=DATASET_DIR):
    ...
    fig, axes = plt.subplots(len(APPLIANCES), 2, figsize=(14, 4 * len(APPLIANCES)))
    fig.suptitle('APR-REFIT-House3-dataset -- Train vs Validation vs Test comparison', fontsize=13)
