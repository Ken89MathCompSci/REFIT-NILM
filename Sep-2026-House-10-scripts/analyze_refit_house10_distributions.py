"""
On-duration and time-of-day distribution analysis for APR-REFIT-House10-dataset/,
broken down per split (train / validation / test).

Ported from APR-REFIT-Scripts/analyze_refit_house2_distributions.py to
House 10's own dataset -- REFIT House 10's counterpart to
Sep-2026-UKDALE-scripts/analyze_apr_new_house5_distributions.py (that
script's House 5 is UKDALE House 5, an unrelated dataset). Already
matches that script's structure (per-split ON-duration + time-of-day +
cross-split coverage check); REFIT needs no per-appliance threshold table
since all four channels read a clean 0 W when off (uniform 10 W threshold),
unlike the UKDALE House 5 script's microwave/washing_machine standby-floor
artefacts.
...
Splits (all House 10, chronologically contiguous, no gap/overlap):
    train      2013-12-08 to 2014-01-06  (30 days)
    validation 2014-01-07 to 2014-01-13  (7 days)
    test       2014-01-14 to 2014-01-20  (7 days)

Per split, per appliance:
    1. Threshold at 10 W (this repo's established uniform-threshold
       convention, same as House 2/3/5) to get a binary ON/OFF state per
       8s sample.
...
trains on, or whether a split's appliance behaviour has drifted -- this
mattered for House 2 (a stale window left validation/test almost devoid of
microwave activity before the window-selection fix); worth checking fresh
for House 10 rather than assuming it's fine just because the overall
min-activation score is decent.

Outputs (written into APR-REFIT-House10-dataset/):
...
def load_splits(dataset_dir=DATASET_DIR):
    ...
    dfs = {}
    for name in SPLITS:
        path = os.path.join(dataset_dir, f'REFIT_HF_{name}.csv')
...
def plot_split(results, split_name, out_dir=DATASET_DIR):
    ...
    fig.suptitle(f'APR-REFIT-House10-dataset -- ON-Duration & Time-of-Day '
...
def plot_comparison(all_results, out_dir=DATASET_DIR):
    ...
    fig, axes = plt.subplots(len(APPLIANCES), 2, figsize=(14, 4 * len(APPLIANCES)))
    fig.suptitle('APR-REFIT-House10-dataset -- Train vs Validation vs Test comparison', fontsize=13)
