"""
test_tcn_refit_house10_dataset_v2.py
=====================================
REFIT House 10 port of Sep-2026-UKDALE-scripts/test_tcn_apr_new_house5_dataset_v2.py
...
House 10 -- this v2 script is ported directly from House 2's v2 script, with
...
ADAPTIVE PER-APPLIANCE THRESHOLDS (train-locked)
    ...
    values on the actual REFIT House 10 train split (2013-12-08 to 2014-01-06):
        dishwasher=21.0W, fridge=10.0W, microwave=36.0W, washing_machine=23.0W
    These differ from the uniform 10 W threshold used by
    test_gru/lnn/lstm_refit_house10_dataset.py -- same known threshold
...
POS_WEIGHT_CLAMP sized to REFIT House 10's actual class imbalance
    ...
    The upper clamp is set to 210 (close to House 3's 200, well under House
    2's 270) -- grounded in a direct measurement of this dataset rather than
    reused from another house.
...
class GatedTCNModel(nn.Module):
    ...
    GatedTCNModel, adapted to REFIT House 10's pos_weight clamp.
    """
...
def train_tcn_on_appliance(data_dict, appliance_name, thresholds, window_size=100,
                            ...
                            save_dir='models/tcn_refit_house10_dataset_v2'):
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"tcn_refit_house10_v2_{appliance_name}_best.pth")
            save_model(model,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"tcn_refit_house10_v2_{appliance_name}_metrics.png"),
    ...
    with open(os.path.join(save_dir, f'tcn_refit_house10_v2_{appliance_name}_history.json'),
...
def test_tcn_on_all_appliances(window_size=100, num_channels=None, kernel_size=3,
    ...
    base_save_dir = os.path.join(
        os.path.dirname(__file__), '..', 'models', f"tcn_refit_house10_dataset_v2_{timestamp}")
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir, f"tcn_refit_house10_v2_{appliance_name}_best.pth"),
    ...
    print(f"\nGated TCN v2 APR-REFIT-House10-dataset testing completed. Results saved to {base_save_dir}")
...
if __name__ == "__main__":
    print("Testing Gated TCN v2 on APR-REFIT-House10-dataset...")
    ...
    print(f"\nSummary of Gated TCN v2 testing on APR-REFIT-House10-dataset:")
