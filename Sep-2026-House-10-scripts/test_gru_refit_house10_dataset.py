"""
GRU baseline for NILM -- APR-REFIT-House10-dataset/ splits.

Ported from APR-REFIT-Scripts/test_gru_refit_house2_dataset.py (already
matching this pattern) to House 10's own dataset -- this is REFIT House
10's counterpart to the Sep-2026-UKDALE-scripts/ folder. Nothing to port
here beyond the dataset path: this script already implements the baseline
pattern used by that folder's test_gru/lnn/lstm scripts (midpoint-aligned
sequences, best-checkpoint restore before test).
The dataset-specific difference is REFIT's uniform 10 W threshold vs UKDALE
House 5's per-appliance thresholds (10/50/200/20 W) -- REFIT's four
appliance channels all read a clean 0 W when off, unlike UKDALE House 5's
microwave/washer standby-floor artefacts.

Derived from Old-LNN_Algorithms/test_gru_ukdale_specific_splits.py with three changes:

1. DATA SOURCE -- reads directly from APR-REFIT-House10-dataset/ CSV files instead
   ...
   Appliances: dishwasher, fridge, microwave, washing_machine (all House 10 -- no
...
3. UNIFORM 10 W THRESHOLD -- all appliances use 10 W (the old script special-
   ...
   this repo's House-2-only datasets.
...
def load_data(dataset_dir=DATASET_DIR):
    """Load train / validation / test from APR-REFIT-House10-dataset/ CSV files."""
...
def train_gru_on_appliance(data_dict, appliance_name, window_size=100,
                            ...
                            save_dir='models/gru_refit_house10_dataset'):
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"gru_refit_house10_{appliance_name}_best.pth")
            save_model(model,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"gru_refit_house10_{appliance_name}_metrics.png"),
    ...
    with open(os.path.join(save_dir, f'gru_refit_house10_{appliance_name}_history.json'),
...
def test_gru_on_all_appliances(window_size=100, hidden_size=128, num_layers=2,
    ...
    base_save_dir = os.path.join(
        os.path.dirname(__file__), '..', 'models', f"gru_refit_house10_dataset_{timestamp}")
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir, f"gru_refit_house10_{appliance_name}_best.pth"),
    ...
    print(f"\nGRU APR-REFIT-House10-dataset testing completed. Results saved to {base_save_dir}\n")
...
if __name__ == "__main__":
    print("Testing GRU on APR-REFIT-House10-dataset...")
    ...
    print(f"\nSummary of GRU testing on APR-REFIT-House10-dataset:")
