"""
Liquid Neural Network (LNN) baseline for NILM -- APR-REFIT-House10-dataset/ splits.

Ported from APR-REFIT-Scripts/test_lnn_refit_house2_dataset.py (already
matching this pattern) to House 10's own dataset -- REFIT House 10's
counterpart to the Sep-2026-UKDALE-scripts/ folder's
test_lnn_apr_new_house5_dataset.py. Already matches that folder's baseline
pattern; only the threshold convention differs (REFIT: uniform 10 W across
all four clean-off channels vs UKDALE House 5's per-appliance
10/50/200/20 W, needed there for standby-floor artefacts REFIT doesn't have).

Derived from Old-LNN_Algorithms/test_lnn_ukdale_specific_splits.py with the same
three changes described in test_gru_refit_house10_dataset.py:
  1. Reads APR-REFIT-House10-dataset/ CSVs instead of pickled UKDALE splits
     (House 10, train 2013-12-08..2014-01-06 / val 2014-01-07..2014-01-13 / test 2014-01-14..2014-01-20).
...
def train_on_appliance(data_dict, appliance_name, window_size=100,
                        ...
                        save_dir='models/lnn_refit_house10_dataset'):
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"lnn_refit_house10_{appliance_name}_best.pth")
            save_model(model,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"lnn_refit_house10_{appliance_name}_metrics.png"),
    ...
    with open(os.path.join(save_dir, f'lnn_refit_house10_{appliance_name}_history.json'),
...
def test_on_all_appliances(window_size=100, hidden_size=64, dt=0.1,
    ...
    base_save_dir = os.path.join(
        os.path.dirname(__file__), '..', 'models', f"lnn_refit_house10_dataset_{timestamp}")
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir, f"lnn_refit_house10_{appliance_name}_best.pth"),
    ...
    print(f"\nLiquidNetworkModel APR-REFIT-House10-dataset testing completed. "
...
if __name__ == "__main__":
    print("Testing LiquidNetworkModel on APR-REFIT-House10-dataset...")
    ...
    print(f"\nSummary of LiquidNetworkModel testing on APR-REFIT-House10-dataset:")
