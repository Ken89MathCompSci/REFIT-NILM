"""
combined_pinn_lnn_refit_house10_dataset.py
=============================================
Dual-head PINN + Gated-Detection LNN for NILM -- APR-REFIT-House10-dataset/ splits.

Ported from Sep-2026-House-2-scripts/combined_pinn_lnn_refit_house2_dataset.py
(the ramp-epoch version, itself already updated from the original
APR-REFIT-Scripts/combined_pinn_lnn_refit_house2_dataset.py) to House 10's own
dataset. Architecture, losses, ramp-epoch loss schedule, and Seq2Seq window
construction are unchanged; the only house-specific change is:

POS_WEIGHT_CLAMP measured directly on House 10's own train split
   (2013-12-08 to 2014-01-06), not reused from House 2. Measured off:on
   ratios at the adaptive threshold on this train split: dishwasher ~13:1,
   fridge ~3:1, microwave ~198:1, washing_machine ~11:1. Close to House 3's
   ~187:1 and well under House 2's ~261:1 worst-case microwave imbalance.
   Clamped to (1.0, 210.0) accordingly.

DATA SOURCE -- default --dataset-dir points at APR-REFIT-House10-dataset/:
    APR-REFIT-House10-dataset/REFIT_HF_train.csv      (House 10, 2013-12-08 to 2014-01-06, 30 days)
    APR-REFIT-House10-dataset/REFIT_HF_validation.csv (House 10, 2014-01-07 to 2014-01-13, 7 days)
    APR-REFIT-House10-dataset/REFIT_HF_test.csv       (House 10, 2014-01-14 to 2014-01-20, 7 days)
...
fridge compressor cycle and was carried over unchanged for House 10; run
analyze_refit_house10_distributions.py if you want this house's own
per-appliance ON-duration distribution to double-check WIN is still generous
enough (House 10's fridge alone runs at 31% duty over its full history, per
...
THRESHOLD_MIN     = 10.0
CYCLING_P5_W      = 80.0
POS_WEIGHT_CLAMP  = (1.0, 210.0)  # House 10 microwave measures ~198:1 off:on
                                   # at its adaptive threshold on the train
                                   # split (see header) -- close to House 3's
                                   # 200.0, well under House 2's 270.0

APPLIANCES  = ['dishwasher', 'fridge', 'microwave', 'washing_machine']
AGG_COL     = 'aggregate'

DEFAULT_DATASET_DIR = os.path.join(os.path.dirname(__file__), '..', 'APR-REFIT-House10-dataset')
...
def load_data(dataset_dir: str) -> dict:
    print(f"Loading APR-REFIT-House10-dataset CSV data from '{dataset_dir}' ...")
...
def train(data_dict: dict, save_dir: str,
    ...
    cfg = {
        'dataset': 'APR-REFIT-House10-dataset (H10, train 2013-12-08..2014-01-06 / '
                   'val 2014-01-07..2014-01-13 / test 2014-01-14..2014-01-20)',
    ...
    with open(os.path.join(save_dir, 'combined_pinn_lnn_refit_house10_dataset_results.json'), 'w') as f:
...
def _plot_loss(history, save_dir):
    ...
    fig.suptitle('CombinedPINNAdvancedLNN (APR-REFIT-House10-dataset) -- Loss Curves')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'combined_pinn_lnn_refit_house10_dataset_loss.png'),
...
def _plot_metrics(history, test_metrics, save_dir):
    ...
    fig.suptitle('CombinedPINNAdvancedLNN (APR-REFIT-House10-dataset) -- Per-Appliance Validation Metrics')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'combined_pinn_lnn_refit_house10_dataset_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == '__main__':
    ...
    save_dir  = os.path.join(
        ...
        f'combined_pinn_lnn_refit_house10_dataset_{timestamp}'
    )
