"""
nilm_sfra_svm_house10.py
========================
REFIT House 10 port of Sep-2026-UKDALE-scripts/nilm_sfra_svm_house5.py -- an
SVM baseline for NILM on APR-REFIT-House10-dataset (House 10, 2013-12-08 to
2014-01-20). No equivalent SVM script existed for REFIT House 10 before this;
it is new here, not an update to an existing script.
...
Features: the same 8-channel aggregate features used by
combined_pinn_lnn_refit_house10_dataset.py (raw, median, EMA, residual,
delta_raw, delta_smooth, rolling_mean, rolling_std), Z-scored with
StandardScaler fit on the training split only.

Adaptive thresholds are computed on train only and reused for test (same
train-locked convention as combined_pinn_lnn_refit_house10_dataset.py).
Measured values on the actual train split: dishwasher=21.0W, fridge=10.0W,
microwave=36.0W, washing_machine=23.0W -- off:on ratios at these thresholds
are dishwasher ~13:1, fridge ~3:1, microwave ~198:1, washing_machine ~11:1,
all much less extreme than the House 5 UKDALE origin's (up to ~1587:1 for
microwave), so the generic (1.0, 10.0) class-weight clamp is left unchanged
here -- it bites hardest for microwave (198 -> 10), same as it does across
every REFIT house analyzed so far.
...
POS_WEIGHT_CLAMP  = (1.0, 10.0)   # clamp ON class weight to avoid precision collapse
                                   # (e.g. dishwasher's ~12:1 raw ratio -> SVC
                                   # predicting ON everywhere without a clamp)

DEFAULT_DATASET_DIR = os.path.join(os.path.dirname(__file__), '..', 'APR-REFIT-House10-dataset')


# ---------------------------------------------------------------------------
# Feature extraction  (identical to combined_pinn_lnn_refit_house10_dataset.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Adaptive thresholds  (identical to combined_pinn_lnn_refit_house10_dataset.py)
# ---------------------------------------------------------------------------
...
def load_data(dataset_dir: str) -> dict:
    print(f"Loading APR-REFIT-House10-dataset from '{dataset_dir}' ...")
...
def main(dataset_dir: str = DEFAULT_DATASET_DIR, tune: bool = False,
    ...
    for app in APPLIANCES:
        ...
        # (e.g. dishwasher ~12:1 -> SVC predicts ON everywhere). Clamp mirrors the
    ...
    out_path  = os.path.join(dataset_dir, f'svm_house10_results_{timestamp}.json')
