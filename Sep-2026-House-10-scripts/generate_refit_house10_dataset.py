"""
Generates APR-REFIT-House10-dataset/ -- multi-day REFIT House 10 splits, same
...
REFIT House 10 appliance channels (processed REFITPowerData111215/House10.csv),
confirmed against the official MetaData_Tables.xlsx (not inferred from power
signatures -- an earlier signature-based guess for this house incorrectly
placed fridge on col2, which is actually a Toaster; the official metadata
corrects this):
    Columns (no header): Unix, Aggregate, App1-App9
    fridge           -> col 4  (Appliance4 = Fridge-Freezer)
    washing_machine  -> col 5  (Appliance5 = Washing Machine, Beko)
    dishwasher       -> col 6  (Appliance6 = Dishwasher, AEG)
    microwave        -> col 8  (Appliance8 = Microwave)
    (col1 = Magimix/Blender, col2 = Toaster, col3 = Chest Freezer, col7 = TV,
    col9 = K Mix -- all skipped)
...
Full history is 2013-11-20 to 2015-06-30 (~586 days). The selected window
landed on 2013-12-08 to 2014-01-20, with a worst-case per-split activation
floor of 1,273 samples (moderately richer than House 2's 1,151).
...
OUT_DIR = os.path.join(BASE, "..", "APR-REFIT-House10-dataset")
os.makedirs(OUT_DIR, exist_ok=True)

# House 10 channel mapping, per MetaData_Tables.xlsx "House 10" sheet:
#   Appliance4 (col4) = Fridge-Freezer    -> fridge
#   Appliance5 (col5) = Washing Machine   -> washing_machine
#   Appliance6 (col6) = Dishwasher        -> dishwasher
#   Appliance8 (col8) = Microwave         -> microwave
H10_COL_MAP = {
    "aggregate":       "agg",
    "fridge":          "app4",
    "washing_machine": "app5",
    "dishwasher":      "app6",
    "microwave":       "app8",
}
...
def scan_best_window(raw_df: pd.DataFrame) -> dict:
    ...
    if n_days_total < total_days:
        raise ValueError(
            f"REFIT House 10 has only {n_days_total} days of data; "
...
def generate_split(raw_df: pd.DataFrame, name: str,
    ...
    print(f"\n{'='*60}")
    print(f"Processing split: {name}  (House 10, {start} -> {end})")
    print(f"{'='*60}")

    cols = {}
    for label, col in H10_COL_MAP.items():
...
if __name__ == "__main__":
    csv_path = os.path.join(REFIT, "House10.csv")
    ...
    print(f"Loading REFIT House 10: {csv_path}")
