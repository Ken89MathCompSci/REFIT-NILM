"""
test_resnet_refit_house5_dataset_v2.py
========================================
REFIT House 5 port of Sep-2026-UKDALE-scripts/test_resnet_apr_new_house5_dataset_v2.py
-- the "v2" gated-head ResNet architecture, ported to this repo's
APR-REFIT-House5-dataset/ splits. Unlike House 2 (which has both an old
un-gated v1 script and this v2 gated version), no v1 ResNet baseline exists
for House 5 -- this v2 script is ported directly from House 2's v2 script,
...
ADAPTIVE PER-APPLIANCE THRESHOLDS (train-locked)
    ...
    combined_pinn_lnn_refit_house5_dataset.py and nilm_sfra_svm_house5.py,
    ...
    on the actual REFIT House 5 train split (2014-08-15 to 2014-09-13):
        dishwasher=24.0W, fridge=22.0W, microwave=21.0W, washing_machine=23.0W
    These differ from the uniform 10 W threshold used by
    test_gru/lnn/lstm_refit_house5_dataset.py -- REFIT's channels are clean
...
POS_WEIGHT_CLAMP sized to REFIT House 5's actual class imbalance
    Measured directly from the current APR-REFIT-House5-dataset/REFIT_HF_train.csv
    at the adaptive thresholds above (off:on sample ratio):
        dishwasher ~13:1, fridge ~1:1, microwave ~56:1, washing_machine ~7:1
    House 5 has the richest, best-balanced data of any REFIT house analyzed
    in this repo (microwave alone is ON 1.8% of train samples, vs House 2's
    0.4%) -- its worst-case imbalance (~56:1 for microwave) is comfortably
    below House 2's ~261:1. The upper clamp is set to 60 so the gate's BCE
    loss isn't starved for microwave while staying grounded in this house's
    actual measured ratio rather than reused from another house's numbers.
...
DATASET_DIR = os.path.join(os.path.dirname(__file__), '..', 'APR-REFIT-House5-dataset')
APPLIANCES  = ['dishwasher', 'fridge', 'microwave', 'washing_machine']

# Adaptive-threshold constants (identical formula to combined_pinn_lnn_refit_house5_dataset.py)
THRESHOLD_DELTA   = 20.0
THRESHOLD_LOW_PCT = 0.05
THRESHOLD_MIN     = 10.0
CYCLING_P5_W      = 80.0
POS_WEIGHT_CLAMP  = (1.0, 60.0)   # REFIT House 5 microwave measures ~56:1 off:on
                                   # at its adaptive threshold on the train split --
                                   # see header for the measured ratios of all four
                                   # appliances. Well below House 2's raised 270.0.
...
def compute_adaptive_thresholds(df: pd.DataFrame) -> dict:
    """Identical formula to combined_pinn_lnn_refit_house5_dataset.py."""
    thresholds = {}
...
class GatedResNetModel(nn.Module):
    ...
    GatedResNetModel, adapted to REFIT House 5's pos_weight clamp.
    """
...
def train_resnet_on_appliance(data_dict, appliance_name, thresholds, window_size=100,
                               ...
                               save_dir='models/resnet_refit_house5_dataset_v2'):
    if layers is None:
        layers = [2, 2, 2]
    ...
    # as GatedEventLoss in combined_pinn_lnn_refit_house5_dataset.py
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"resnet_refit_house5_v2_{appliance_name}_best.pth")
            save_model(model,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"resnet_refit_house5_v2_{appliance_name}_metrics.png"),
                dpi=150, bbox_inches='tight')
    plt.close()

    config = {
        'appliance': appliance_name,
        'dataset': 'APR-REFIT-House5-dataset',
        'model': 'GatedResNetModel',
        'version': 'v2',
        'fixes': {
    ...
    with open(os.path.join(save_dir, f'resnet_refit_house5_v2_{appliance_name}_history.json'),
...
def test_resnet_on_all_appliances(window_size=100, base_width=32, layers=None,
    ...
    base_save_dir = os.path.join(
        os.path.dirname(__file__), '..', 'models', f"resnet_refit_house5_dataset_v2_{timestamp}")
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir, f"resnet_refit_house5_v2_{appliance_name}_best.pth"),
    ...
    summary = {
        ...
        'version': 'v2',
        'dataset_splits': {
            'training':   {'house': 5, 'start': '2014-08-15', 'end': '2014-09-13'},
            'validation': {'house': 5, 'start': '2014-09-14', 'end': '2014-09-20'},
            'testing':    {'house': 5, 'start': '2014-09-21', 'end': '2014-09-27'}
        },
        'window_size': window_size,
    ...
    print(f"\nGated ResNet v2 APR-REFIT-House5-dataset testing completed. Results saved to {base_save_dir}")
...
if __name__ == "__main__":
    print("Testing Gated ResNet v2 on APR-REFIT-House5-dataset...")
    ...
    print(f"\nSummary of Gated ResNet v2 testing on APR-REFIT-House5-dataset:")
