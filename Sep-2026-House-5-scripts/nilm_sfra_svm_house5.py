...
========================
REFIT House 5 port of Sep-2026-UKDALE-scripts/nilm_sfra_svm_house5.py -- an
SVM baseline for NILM on APR-REFIT-House5-dataset (House 5, 2014-08-15 to
2014-09-27). No equivalent SVM script existed for REFIT House 5 before this;
...
combined_pinn_lnn_refit_house5_dataset.py (raw, median, EMA, residual,
...
train-locked convention as combined_pinn_lnn_refit_house5_dataset.py).
Measured values on the actual train split: dishwasher=24.0W, fridge=22.0W,
microwave=21.0W, washing_machine=23.0W -- off:on ratios at these thresholds
are dishwasher ~13:1, fridge ~1:1, microwave ~56:1, washing_machine ~7:1.
House 5 has the richest, best-balanced data of any REFIT house analyzed in
this repo, but its microwave ratio (~56:1) still exceeds the generic
(1.0, 10.0) class-weight clamp inherited from the UKDALE House 5 script, so
that clamp still bites here too -- just a smaller relative adjustment
(56 -> 10) than House 2's (261 -> 10). Left unchanged since 10.0 was already
tuned for SVC's precision-collapse failure mode, not for any one house's
raw ratio.
...
# ---------------------------------------------------------------------------
# Feature extraction  (identical to combined_pinn_lnn_refit_house5_dataset.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Adaptive thresholds  (identical to combined_pinn_lnn_refit_house5_dataset.py)
# ---------------------------------------------------------------------------
...
def load_data(dataset_dir: str) -> dict:
    print(f"Loading APR-REFIT-House5-dataset from '{dataset_dir}' ...")
