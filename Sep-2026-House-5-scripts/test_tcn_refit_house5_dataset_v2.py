"""
test_tcn_refit_house5_dataset_v2.py
=====================================
REFIT House 5 port of Sep-2026-UKDALE-scripts/test_tcn_apr_new_house5_dataset_v2.py
-- the "v2" gated-head TCN architecture, ported to this repo's
APR-REFIT-House5-dataset/ splits. Unlike House 2 (which has both an old
un-gated v1 script and this v2 gated version), no v1 TCN baseline exists for
House 5 -- this v2 script is ported directly from House 2's v2 script, with
the same two fixes documented there:
...
ADAPTIVE PER-APPLIANCE THRESHOLDS (train-locked)
    ...
    computed on the train split only and reused for val/test. Measured
    values on the actual REFIT House 5 train split (2014-08-15 to 2014-09-13):
        dishwasher=24.0W, fridge=22.0W, microwave=21.0W, washing_machine=23.0W
    These differ from the uniform 10 W threshold used by
    test_gru/lnn/lstm_refit_house5_dataset.py -- same known threshold
    inconsistency between baseline and gated scripts already documented for
    House 2.
...
POS_WEIGHT_CLAMP sized to REFIT House 5's actual class imbalance
    ...
    House 5 is the richest/best-balanced REFIT house analyzed in this repo --
    its worst-case imbalance (~56:1 for microwave) is far below House 2's
    ~261:1. The upper clamp is set to 60, grounded in a direct measurement
    of this dataset rather than reused from another house.
...
restore before test -- matches the UKDALE House 5 v2 script unchanged.
"""
...
# ---------------------------------------------------------------------------
# Gated TCN -- same dilated-TCN trunk as the un-gated TCNModel, but with a
...
class GatedTCNModel(nn.Module):
    ...
    Identical design to Sep-2026-UKDALE-scripts/test_tcn_apr_new_house5_dataset_v2.py's
    GatedTCNModel, adapted to REFIT House 5's pos_weight clamp.
    """
...
def train_tcn_on_appliance(data_dict, appliance_name, thresholds, window_size=100,
                            ...
                            save_dir='models/tcn_refit_house5_dataset_v2'):
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"tcn_refit_house5_v2_{appliance_name}_best.pth")
            save_model(model,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"tcn_refit_house5_v2_{appliance_name}_metrics.png"),
    ...
    with open(os.path.join(save_dir, f'tcn_refit_house5_v2_{appliance_name}_history.json'),
...
def test_tcn_on_all_appliances(window_size=100, num_channels=None, kernel_size=3,
    ...
    base_save_dir = os.path.join(
        os.path.dirname(__file__), '..', 'models', f"tcn_refit_house5_dataset_v2_{timestamp}")
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir, f"tcn_refit_house5_v2_{appliance_name}_best.pth"),
    ...
    print(f"\nGated TCN v2 APR-REFIT-House5-dataset testing completed. Results saved to {base_save_dir}")
...
if __name__ == "__main__":
    print("Testing Gated TCN v2 on APR-REFIT-House5-dataset...")
    ...
    print(f"\nSummary of Gated TCN v2 testing on APR-REFIT-House5-dataset:")
