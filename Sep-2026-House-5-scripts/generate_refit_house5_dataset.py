"""
Generates APR-REFIT-House5-dataset/ -- multi-day REFIT House 5 splits, same
column format as APR-REFIT-House2-dataset/ (timestamp, aggregate, dishwasher,
fridge, microwave, washing_machine) at 8s resolution.

REFIT House 5 appliance channels (processed REFITPowerData111215/House5.csv),
confirmed against the official MetaData_Tables.xlsx (not inferred from power
signatures -- House 5's sheet lists model/manufacturer per channel):
    Columns (no header): Unix, Aggregate, App1-App9
    fridge           -> col 1  (Appliance1 = Fridge-Freezer, Fisher & Paykel)
    washing_machine  -> col 3  (Appliance3 = Washing Machine, AEG)
    dishwasher       -> col 4  (Appliance4 = Dishwasher)
    microwave        -> col 7  (Appliance7 = Microwave)
    (col2 = Tumble Dryer, col5 = Desktop Computer, col6 = TV, col8 = Kettle,
    col9 = Toaster -- all skipped)

REFIT is sampled at 8-second intervals. MAX_FILL = 37 (~5 min = 37.5 x 8s
steps, floored) -- same policy as generate_refit_house2_dataset.py.

A 44-day chronologically contiguous window (30 train + 7 val + 7 test) is
selected automatically by scan_best_window(), maximising the minimum
...
not a selection factor) -- identical scoring to generate_refit_house2_dataset.py.

House 5 has substantially richer appliance data than House 2 across its full
~648-day recording (2013-09-26 to 2015-07-06): microwave alone is ON 6.80% of
the time (vs House 2's 0.56%), giving the window scan far more to work with.
The selected window landed on 2014-08-15 to 2014-09-27, with a worst-case
per-split activation floor of 11,471 samples -- about 10x House 2's 1,151.

Caveat worth knowing before trusting this at face value: House 5's microwave
segments have an unusually long tail (median ON duration 1.2 min, but mean
15.6 min and p95 ~70 min across the full history) -- consistent with real
heavy use, but also worth a visual/time-series spot-check since a long tail
like that can also indicate the channel occasionally captures something else
on the same circuit. The generated CSV carries raw power values regardless,
so this doesn't block generation -- just something to keep in mind before
fully trusting a trained microwave model on this house.

Gap-filling policy: forward-fill gaps <= 5 min (37 x 8s steps), then
zero-fill. Appliance power is clipped to >= 0 W after filling.
"""
...
OUT_DIR = os.path.join(BASE, "..", "APR-REFIT-House5-dataset")
os.makedirs(OUT_DIR, exist_ok=True)

# House 5 channel mapping, per MetaData_Tables.xlsx "House 5" sheet:
#   Appliance1 (col1) = Fridge-Freezer    -> fridge
#   Appliance3 (col3) = Washing Machine   -> washing_machine
#   Appliance4 (col4) = Dishwasher        -> dishwasher
#   Appliance7 (col7) = Microwave         -> microwave
H5_COL_MAP = {
    "aggregate":       "agg",
    "fridge":          "app1",
    "washing_machine": "app3",
    "dishwasher":      "app4",
    "microwave":       "app7",
}
...
THRESHOLD = 10.0  # W -- ON/OFF boundary used during scanning, matches House 2
...
def scan_best_window(raw_df: pd.DataFrame) -> dict:
    ...
    Identical scoring to generate_refit_house2_dataset.py.
    """
    ...
    if n_days_total < total_days:
        raise ValueError(
            f"REFIT House 5 has only {n_days_total} days of data; "
...
def generate_split(raw_df: pd.DataFrame, name: str,
    ...
    print(f"\n{'='*60}")
    print(f"Processing split: {name}  (House 5, {start} -> {end})")
    print(f"{'='*60}")

    cols = {}
    for label, col in H5_COL_MAP.items():
...
if __name__ == "__main__":
    csv_path = os.path.join(REFIT, "House5.csv")
    ...
    print(f"Loading REFIT House 5: {csv_path}")
