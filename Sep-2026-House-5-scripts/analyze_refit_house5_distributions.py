"""
On-duration and time-of-day distribution analysis for APR-REFIT-House5-dataset/,
broken down per split (train / validation / test).

Ported from APR-REFIT-Scripts/analyze_refit_house2_distributions.py to
House 5's own dataset -- REFIT House 5's counterpart to
Sep-2026-UKDALE-scripts/analyze_apr_new_house5_distributions.py (that
script's House 5 is UKDALE House 5, an unrelated dataset). Already matches
that script's structure (per-split ON-duration + time-of-day + cross-split
coverage check); REFIT needs no per-appliance threshold table since all
four channels here read a clean 0 W when off (uniform 10 W threshold),
...
Splits (all House 5, chronologically contiguous, no gap/overlap):
    train      2014-08-15 to 2014-09-13  (30 days)
    validation 2014-09-14 to 2014-09-20  (7 days)
    test       2014-09-21 to 2014-09-27  (7 days)

Per split, per appliance:
    1. Threshold at 10 W (this repo's established uniform-threshold
       convention, same as House 2/3/10) to get a binary ON/OFF state per
       8s sample.
...
for House 5 rather than assuming it's fine just because the overall
min-activation score is high.

Outputs (written into APR-REFIT-House5-dataset/):
...
def plot_split(results, split_name, out_dir=DATASET_DIR):
    ...
    fig.suptitle(f'APR-REFIT-House5-dataset -- ON-Duration & Time-of-Day '
...
def plot_comparison(all_results, out_dir=DATASET_DIR):
    ...
    fig, axes = plt.subplots(len(APPLIANCES), 2, figsize=(14, 4 * len(APPLIANCES)))
    fig.suptitle('APR-REFIT-House5-dataset -- Train vs Validation vs Test comparison', fontsize=13)
