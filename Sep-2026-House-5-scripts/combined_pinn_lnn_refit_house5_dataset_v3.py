"""
combined_pinn_lnn_refit_house5_dataset_v3.py
=============================================
...
combined_pinn_lnn_refit_house5_dataset.py (unchanged) and from
combined_pinn_lnn_refit_house5_dataset_v2.py (also left unchanged) rather
than editing either in place, so all three remain independently
runnable/comparable -- same pattern as
combined_pinn_lnn_refit_house2_dataset_v3.py vs its House 2 v2/original.

WHY V2 EXISTED (House 5 specific motivation, unchanged from that file)
   ...
   ResNet v2 got F1=0.274/recall=0.708 on the same data. That ruled out a
   data/appliance oddity and pointed at the shared trunk starving out
   whichever appliance has the weakest *combined* signal: washing_machine's
   off:on ratio (~7:1) is mild enough that MSE alone can mostly ignore it,
   but not severe enough to earn a pos_weight large enough to force
   attention onto it the way microwave's ~56:1 does. v2's fix was wider
   per-appliance heads (HEAD_HIDDEN=32, kept unchanged here) + LAMBDA_EVENT
   raised 0.05 -> 0.25.

WHY V3 EXISTS -- v2 WAS RUN, AND THIS IS WHAT ACTUALLY HAPPENED
   washing_machine improved dramatically, exactly as hypothesized:
   F1 0.015 -> 0.200 (~13x), recall 0.008 -> 0.240 (~31x). But macro F1
   still dropped overall (0.389 -> 0.371), because the fix wasn't
   selective -- LAMBDA_EVENT is a single global multiplier, so raising it
   pushed the gate to fire more liberally for *every* appliance, not just
   the starved one. Fridge and microwave both show recall-up/precision-down
   shifts (fridge FP 8,665 -> 16,918; microwave FP 278 -> 11,928, a 43x
   jump for only a modest recall gain 0.104 -> 0.192). Microwave is the
   real casualty: it was already reasonably precise before (0.409) even at
   low recall, and v2 traded almost all of that away (precision collapsed
   to 0.029). Dishwasher also regressed (F1 0.510 -> 0.405) on both
   precision and recall, which a uniform "fire more liberally" story
   doesn't fully explain -- plausibly just this single run's training
   trajectory rather than a systematic effect (no seed averaging here).
   Diagnosis: GatedEventLoss sums each appliance's pos_weight-scaled BCE
   term and divides by n_apps, but pos_weight only balances
   positive/negative *within* an appliance -- it doesn't equalize how much
   each appliance's loss *magnitude* contributes to the shared gradient.
   Microwave's pos_weight=56 naturally produces a larger BCE value than
   washing_machine's pos_weight=7, so raising the outer LAMBDA_EVENT
   multiplier amplifies whichever appliance already had the loudest
   signal (microwave), not specifically the quietest one (washing_machine)
   -- washing_machine's rescue was a side effect of the general "fire more"
   push, not a targeted one, and every other appliance paid for it too.

V3 FIX -- pos_weight-normalized event loss + moderated LAMBDA_EVENT
   GatedEventLoss now divides each appliance's BCE term by its own
   pos_weight before summing, so every appliance contributes comparably to
   the gradient regardless of imbalance severity -- pos_weight still does
   its original job (within-appliance class balance) but no longer also
   sets between-appliance priority. LAMBDA_EVENT moderated 0.25 -> 0.15
   (down from v2, still above the original 0.05) since normalization
   already changes the loss scale the outer multiplier applies to. Wider
   heads (HEAD_HIDDEN=32) kept unchanged from v2 -- that part of the fix
   was never implicated in the overcorrection, only the loss weighting was.
   Goal: keep washing_machine's rescue while not sacrificing microwave's
   precision or dishwasher's balance to get it.
...
LAMBDA_PHYS   = 0.01
LAMBDA_EVENT  = 0.15   # moderated from v2's 0.25 -- see header. Now that
                       # GatedEventLoss normalizes each appliance's BCE term
                       # by its own pos_weight before summing, this
                       # multiplier no longer needs to compensate for the
                       # loudest-pos_weight appliance (microwave) dominating
                       # the shared gradient; a smaller push is enough.
EPSILON_W     = 50.0
...
class GatedEventLoss(nn.Module):
    ...
    v3 change (see header): each appliance's pos_weight-scaled BCE term is
    divided by that appliance's own pos_weight before summing. pos_weight
    still does its original job inside each appliance's own BCE (correcting
    for that appliance's positive/negative imbalance), but this stops the
    *magnitude* of pos_weight from also silently setting between-appliance
    priority in the shared gradient -- without the normalization, microwave
    (pos_weight=56 here) contributes a much larger loss, and therefore much
    more gradient pull on the shared trunk, than washing_machine
    (pos_weight=7), for a comparable level of "wrongness". That's the
    mechanism that let v2's higher LAMBDA_EVENT rescue washing_machine while
    overcorrecting microwave.
    """
...
def train(data_dict: dict, save_dir: str,
    ...
    with open(os.path.join(save_dir, 'combined_pinn_lnn_refit_house5_dataset_v3_results.json'), 'w') as f:
...
def _plot_loss(history, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'combined_pinn_lnn_refit_house5_dataset_v3_loss.png'),
...
def _plot_metrics(history, test_metrics, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'combined_pinn_lnn_refit_house5_dataset_v3_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == '__main__':
    ...
    save_dir  = os.path.join(
        ...
        f'combined_pinn_lnn_refit_house5_dataset_v3_{timestamp}'
    )
